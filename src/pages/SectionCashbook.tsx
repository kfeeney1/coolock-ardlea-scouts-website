...
import { createFinanceTransaction, createFinanceTransfer, loadFinanceTransactions, reverseFinanceTransaction } from "../services/financeLedger";
import { createFinanceReconciliation, loadFinanceReconciliations } from "../services/financeReconciliations";
import {
  DEFAULT_FINANCE_CATEGORIES, calculateLedgerBalanceCents, reconcileFinanceFloat, signedAmountCents,
  type FinanceReconciliationRecord, type FinanceTransaction, type FinanceTransactionType,
...
export default function SectionCashbook() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      console.error("Unable to load section cashbook:", refreshError); setError("Unable to load this section cashbook.");
  ...
  const submit = async () => {
    ...
    if (amountCents === null || amountCents <= 0) { setError("Enter an amount greater than zero with no more than two decimal places."); return; }
    setSaving(true); setError("");
    try {
      await createFinanceTransaction({ section, type, amountCents, category, description, transactionDate, sourceTransactionId: "", reversalOfTransactionId: "" });
      setAmount(""); setDescription(""); await refresh();
    } catch (submitError) { console.error("Unable to add cashbook transaction:", submitError); setError(submitError instanceof Error ? submitError.message : "Unable to save this transaction."); }
    finally { setSaving(false); }
  };

  const saveTransfer = async () => {
    const amountCents = eurosToCents(transferAmount);
    if (amountCents === null || amountCents <= 0) { setError("Enter a transfer amount greater than zero with no more than two decimal places."); return; }
    setSaving(true); setError("");
    try {
      await createFinanceTransfer({ fromSection: section, toSection: transferTo, amountCents, description: transferDescription, transactionDate: transferDate });
      setTransferAmount(""); setTransferDescription(""); await refresh();
    } catch (transferError) { console.error("Unable to transfer section finance:", transferError); setError(transferError instanceof Error ? transferError.message : "Unable to save this transfer."); }
    finally { setSaving(false); }
  };
  ...
  const saveReconciliation = async () => {
    ...
    catch (reconciliationError) { console.error("Unable to reconcile section cash:", reconciliationError); setError(reconciliationError instanceof Error ? reconciliationError.message : "Unable to save this reconciliation."); }
    finally { setSaving(false); }
  };
      ...
      <Stack spacing={3}>
        <Paper elevation={2} sx={{ p: { xs: 2.5, md: 4 } }}>
          ...
          <Box sx={{ mt: 3, display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr auto" }, gap: 2, alignItems: "center" }}>
            ...
            <Paper variant="outlined" sx={{ px: 3, py: 2, minWidth: 190 }}><Typography variant="caption" color="text.secondary">Calculated balance</Typography><Typography variant="h5" sx={{ fontWeight: 800 }}>{formatEuro(balanceCents)}</Typography></Paper>
          </Box>
        </Paper>
        ...
        <Paper elevation={2} sx={{ p: { xs: 2.5, md: 4 } }}>
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(2, minmax(0, 1fr))" }, gap: 2 }}>
            <FormControl><InputLabel id="finance-type-label">Type</InputLabel><Select labelId="finance-type-label" id="finance-type" label="Type" value={type} onChange={(event) => setType(event.target.value as FinanceTransactionType)}><MenuItem value="opening-float">Opening float</MenuItem><MenuItem value="income">Money in</MenuItem><MenuItem value="expense">Expense</MenuItem></Select></FormControl>
            <TextField label="Amount (€)" inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value)} />
            <FormControl><InputLabel id="finance-category-label">Category</InputLabel><Select labelId="finance-category-label" id="finance-category" label="Category" value={category} onChange={(event) => setCategory(event.target.value)}>{DEFAULT_FINANCE_CATEGORIES.map((item) => <MenuItem key={item} value={item}>{item}</MenuItem>)}</Select></FormControl>
        ...
        {isAllSectionsRole && <Paper elevation={2} sx={{ p: { xs: 2.5, md: 4 } }}>
          <Typography variant="h6" sx={{ fontWeight: 800 }}>Transfer between sections</Typography><Typography color="text.secondary" sx={{ mt: 0.5, mb: 2 }}>Transfers create linked money-out and money-in entries in one atomic write, so one side cannot be saved without the other.</Typography>
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(2, minmax(0, 1fr))" }, gap: 2 }}>
            <TextField label="From section" value={section} slotProps={{ input: { readOnly: true } }} />
            <FormControl><InputLabel id="finance-transfer-to-label">To section</InputLabel><Select labelId="finance-transfer-to-label" id="finance-transfer-to" label="To section" value={transferTo} onChange={(event) => setTransferTo(event.target.value)}>{transferDestinations.map((item) => <MenuItem key={item} value={item}>{item}</MenuItem>)}</Select></FormControl>
            <TextField label="Transfer amount (€)" inputMode="decimal" value={transferAmount} onChange={(event) => setTransferAmount(event.target.value)} /><TextField type="date" label="Transfer date" value={transferDate} onChange={(event) => setTransferDate(event.target.value)} slotProps={{ inputLabel: { shrink: true } }} /><TextField label="Transfer description" value={transferDescription} onChange={(event) => setTransferDescription(event.target.value)} sx={{ gridColumn: { md: "1 / -1" } }} />
          </Box><Button variant="contained" color="secondary" disabled={saving || !section || !transferTo} onClick={() => void saveTransfer()} sx={{ mt: 2, minHeight: 44 }}>{saving ? "Saving…" : "Transfer money"}</Button>
        </Paper>}

        <Paper elevation={2} sx={{ p: { xs: 2.5, md: 4 } }}>
          <Typography variant="h6" sx={{ fontWeight: 800 }}>Cash reconciliation</Typography><Typography color="text.secondary" sx={{ mt: 0.5, mb: 2 }}>Count the physical cash and compare it with the ledger. A difference is recorded for investigation; it does not alter the balance automatically.</Typography>
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, gap: 2 }}><TextField label="Physical cash counted (€)" inputMode="decimal" value={countedCash} onChange={(event) => setCountedCash(event.target.value)} /><TextField label="Reconciliation note" value={reconciliationNote} onChange={(event) => setReconciliationNote(event.target.value)} placeholder="Required if there is a difference" /></Box>
        ...
        <Paper elevation={2} sx={{ p: { xs: 2.5, md: 4 } }}>
          ...
          {loading ? <Typography color="text.secondary">Loading cashbook…</Typography> : transactions.length === 0 ? <Alert severity="info">No cashbook transactions have been recorded for this section.</Alert> : <Stack spacing={1.5}>{transactions.map((transaction) => {
            const signed = signedAmountCents(transaction); const isAdjustment = transaction.type === "adjustment"; const isTransfer = transaction.type === "transfer-in" || transaction.type === "transfer-out"; const isCorrected = reversedIds.has(transaction.id);
            return <Paper key={transaction.id} variant="outlined" sx={{ p: 2 }}><Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, justifyContent: "space-between", gap: 1.5 }}><Box><Stack direction="row" spacing={1} useFlexGap sx={{ flexWrap: "wrap", alignItems: "center" }}><Typography sx={{ fontWeight: 800 }}>{transaction.description}</Typography>{isAdjustment && <Chip size="small" label="Correction" variant="outlined" />}{isTransfer && <Chip size="small" label="Transfer" color="info" variant="outlined" />}{isCorrected && <Chip size="small" label="Corrected" color="warning" variant="outlined" />}</Stack><Typography variant="body2" color="text.secondary">{transaction.transactionDate} · {transaction.category} · {transaction.type}</Typography>{transaction.reversalOfTransactionId && <Typography variant="caption" color="text.secondary">Reverses transaction {transaction.reversalOfTransactionId}</Typography>}{isTransfer && transaction.sourceTransactionId && <Typography variant="caption" color="text.secondary" sx={{ display: "block" }}>Linked transfer {transaction.sourceTransactionId}</Typography>}<FinanceReceiptControl transactionId={transaction.id} section={transaction.section} /></Box><Stack direction={{ xs: "column", sm: "row" }} spacing={1} sx={{ alignItems: { sm: "center" } }}><Typography sx={{ fontWeight: 800 }}>{signed >= 0 ? "+" : "−"}{formatEuro(Math.abs(signed))}</Typography>{!isAdjustment && !isTransfer && !isCorrected && <Button size="small" variant="outlined" color="warning" onClick={() => { setCorrection(transaction); setCorrectionReason(""); setCorrectionDate(today()); }}>Correct entry</Button>}</Stack></Box></Paper>;
          })}</Stack>}
        </Paper>
      </Stack>
      <Dialog open={Boolean(correction)} onClose={() => !saving && setCorrection(null)} fullWidth maxWidth="sm"><DialogTitle>Correct cashbook entry</DialogTitle><DialogContent dividers>{correction && <Stack spacing={2}><Alert severity="warning">This does not edit or delete the original. It creates an equal and opposite adjustment linked to the original transaction.</Alert><Typography><strong>Original:</strong> {correction.description} · {formatEuro(signedAmountCents(correction))}</Typography><TextField type="date" label="Correction date" value={correctionDate} onChange={(event) => setCorrectionDate(event.target.value)} slotProps={{ inputLabel: { shrink: true } }} /><TextField label="Reason / note" value={correctionReason} onChange={(event) => setCorrectionReason(event.target.value)} placeholder={`Correction of ${correction.description}`} multiline minRows={2} /></Stack>}</DialogContent><DialogActions><Button disabled={saving} onClick={() => setCorrection(null)}>Cancel</Button><Button disabled={saving} variant="contained" color="warning" onClick={() => void saveCorrection()}>{saving ? "Saving…" : "Create correction"}</Button></DialogActions></Dialog>
    </Container>
  </Box>;
}
