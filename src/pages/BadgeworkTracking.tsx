...
export default function BadgeworkTracking() {
  ...
  const changeSkill = (nextSkillId: string) => {
    ...
    setDraft(new Map()); setSkillId(nextSkillId); setStageNumber(nextSkill?.stages[0]?.stage ?? 1); setMessage("");
  };
  const changeStage = (nextStage: number) => { if (confirmDiscard()) { setDraft(new Map()); setStageNumber(nextStage); setMessage(""); } };
  ...
  const changeMembers = () => { if (confirmDiscard()) { setDraft(new Map()); setMessage(""); setWorkflowStep("members"); } };
  ...
  const setAward = async (awarded: boolean) => {
    ...
    if (!awarded && !window.confirm(`Remove the Stage ${stage.stage} ${skill.name} award for the selected ${selectedIds.length === 1 ? "child" : "children"}?`)) return;
    setSaving(true); setError(""); setMessage("");
    try {
  ...
  return <Box sx={{ minHeight: "100vh", backgroundColor: "background.default", py: { xs: 3, md: 5 } }}><Container maxWidth="xl">
      ...
      {workflowStep === "badgework" && <Paper elevation={2} sx={{ p: { xs: 2, md: 3 } }}>
        ...
        <Paper variant="outlined" sx={{ p: 2, mb: 2 }} data-testid="badge-award-panel">
          <Stack direction={{ xs: "column", md: "row" }} spacing={1.5} sx={{ justifyContent: "space-between", alignItems: { md: "center" } }}>
            ...
            <Stack direction={{ xs: "column", sm: "row" }} spacing={1}><Button variant="contained" color="success" disabled={saving || !canAward || awardState === "all"} onClick={() => void setAward(true)}>Award badge</Button><Button variant="outlined" color="warning" disabled={saving || awardState === "none" || hasUnsavedChanges} onClick={() => void setAward(false)}>Remove award</Button></Stack>
          </Stack>
