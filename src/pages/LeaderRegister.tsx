...
export default function LeaderRegister() {
    ...
    if (submitted) {
        return <Box sx={{ minHeight: "70vh", backgroundColor: "background.default", py: { xs: 5, md: 8 } }}>
            <Container maxWidth="sm"><Paper elevation={4} sx={{ p: { xs: 3, md: 5 }, textAlign: "center", borderTop: `7px solid ${brandColours.green}` }}>
                <Typography variant="h3" color="secondary">Registration Received</Typography>
                <Alert severity="success" sx={{ mt: 3 }}>Your leader registration request has been sent to the group administrator.</Alert>
                <Typography sx={{ mt: 3 }}>You will not be able to access the Leader Dashboard until an administrator approves your account.</Typography>
                <Button component={Link} to={usingExistingAccount ? "/parent" : "/leader/login"} variant="contained" color="secondary" sx={{ mt: 4 }}>{usingExistingAccount ? "Return to Parent Portal" : "Return to Leader Login"}</Button>
            </Paper></Container>
        </Box>;
    }

    return <Box sx={{ minHeight: "100vh", backgroundColor: "background.default", py: { xs: 4, md: 7 } }}>
        <Container maxWidth="md"><Paper elevation={4} sx={{ overflow: "hidden" }}>
            <Box sx={{ background: `linear-gradient(135deg, ${brandColours.coral}, ${brandColours.navy})`, color: "white", p: { xs: 3, md: 5 }, textAlign: "center" }}>
                ...
                <Typography sx={{ mt: 1.5 }}>{content.group.name}</Typography>
            </Box>
            <Box component="form" onSubmit={submit} noValidate sx={{ p: { xs: 3, md: 5 } }}>
                <Alert severity="info" sx={{ mb: 4 }}>{usingExistingAccount ? "You are already signed in. This request will add Leader Access to the same account after administrator approval — no second login or password is needed." : "Registration creates a pending account request. An existing administrator must approve it before Leader Dashboard access is granted."}</Alert>
                <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr" }, gap: 3 }}>
                    ...
                    <FormControl required error={Boolean(errors.requestedRole)}><InputLabel>Requested role</InputLabel><Select label="Requested role" value={formData.requestedRole} onChange={(e) => { setFormData(c => ({ ...c, requestedRole: e.target.value as RequestedLeaderRole })); clear("requestedRole"); }}><MenuItem value="Scouter">Scouter</MenuItem><MenuItem value="Section Leader">Section Leader</MenuItem><MenuItem value="Group Leader">Group Leader</MenuItem><MenuItem value="Other">Other</MenuItem></Select>{errors.requestedRole && <FormHelperText>{errors.requestedRole}</FormHelperText>}</FormControl>
                    ...
                    {!usingExistingAccount && <TextField required type="password" label="Password" value={formData.password} onChange={(e) => { setFormData(c => ({ ...c, password: e.target.value })); clear("password"); }} error={Boolean(errors.password)} helperText={errors.password ?? "At least 8 characters."} autoComplete="new-password" />}
                    {!usingExistingAccount && <TextField required type="password" label="Confirm password" value={confirmPassword} onChange={(e) => { setConfirmPassword(e.target.value); clear("confirmPassword"); }} error={Boolean(errors.confirmPassword)} helperText={errors.confirmPassword} autoComplete="new-password" />}
                </Box>
                ...
                <FormControl error={Boolean(errors.privacyConfirmed)} sx={{ mt: 3 }}><FormControlLabel control={<Checkbox color="success" checked={formData.privacyConfirmed} onChange={(e) => { setFormData(c => ({ ...c, privacyConfirmed: e.target.checked })); clear("privacyConfirmed"); }} />} label="I confirm that the information supplied is accurate and may be used by the Scout Group to assess this leader access request." />{errors.privacyConfirmed && <FormHelperText>{errors.privacyConfirmed}</FormHelperText>}</FormControl>
                {submitError && <Alert severity="error" sx={{ mt: 3 }}>{submitError}</Alert>}
                <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, gap: 2, justifyContent: "space-between", mt: 4 }}><Button component={Link} to={usingExistingAccount ? "/parent" : "/leader/login"} color="secondary">{usingExistingAccount ? "Back to Parent Portal" : "Back to Login"}</Button><Button type="submit" variant="contained" color="success" disabled={submitting}>{submitting ? "Registering..." : "Request Leader Access"}</Button></Box>
            </Box>
        </Paper></Container>
    </Box>;
}
