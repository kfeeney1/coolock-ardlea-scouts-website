    ...
    if (submitted) {
        return <Box sx={{ minHeight: "70vh", backgroundColor: "background.default", py: { xs: 5, md: 8 } }}>
            <Container maxWidth="sm"><Paper elevation={4} sx={{ p: { xs: 3, md: 5 }, textAlign: "center", borderTop: `7px solid ${brandColours.green}` }}>
                ...
                <Alert severity="success" sx={{ mt: 3 }}>Your leader registration has been sent to the group administrator.</Alert>
                <Typography sx={{ mt: 3 }}>Leader access is reviewed separately. If the same login is also registered as a parent, the administrator will see the match and confirm it before approval.</Typography>
                <Button component={Link} to="/leader/login" variant="contained" color="secondary" sx={{ mt: 4 }}>Return to Leader Login</Button>
            </Paper></Container>
        </Box>;
    }
        ...
        <Container maxWidth="md"><Paper elevation={4} sx={{ overflow: "hidden" }}>
            ...
            <Box component="form" onSubmit={submit} noValidate sx={{ p: { xs: 3, md: 5 } }}>
                ...
                <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr" }, gap: 3 }}>
                    ...
                    {!usingExistingAccount && <TextField required type="password" label="Password" value={formData.password} onChange={(e) => { setFormData(c => ({ ...c, password: e.target.value })); clear("password"); }} error={Boolean(errors.password)} helperText={errors.password ?? "At least 8 characters. Use the same password if this email already has Parent access."} autoComplete="new-password" />}
