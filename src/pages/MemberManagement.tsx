...
const consentExpired = (value: string) =>
...
export default function MemberManagement() {
    ...
    const visibleMembers = useMemo(
        () =>
            members.filter((member) => {
                if (sectionFilter !== "all" && member.section !== sectionFilter) {
                    return false;
                }
    ...
    const save = async () => {
        ...
        try {
            ...
            setMembers((current) =>
                current.map((member) => (member.id === selected.id ? updated : member))
            );
            setSelected(updated);
            setDraft(updated);
    ...
    const addMember = async () => {
        ...
        if (!addDraft.displayName.trim()) {
            setAddError("Member name is required.");
            return;
        }

        if (!addDraft.section) {
            setAddError("Select the member's section.");
            return;
        }
    ...
    const memberForm = (
        value: CreateMemberInput,
        setValue: (value: CreateMemberInput) => void
    ) => (
        <Box
            sx={{
                display: "grid",
        ...
        >
            ...
            <TextField
                ...
                helperText="Use the name leaders normally identify the member by."
            />
            <TextField
                type="date"
                label="Date of birth"
                ...
                onChange={(event) => setValue({ ...value, dateOfBirth: event.target.value })}
            />
            <FormControl required>
                ...
                <Select
                    label="Section"
                    value={value.section}
                    onChange={(event) => setValue({ ...value, section: event.target.value })}
                >
                    {sections
            ...
            <FormControl>
                ...
                <Select
                    ...
                    value={value.status}
                    onChange={(event) =>
                        setValue({ ...value, status: event.target.value as MemberStatus })
                    }
                >
            ...
            <TextField
                ...
                onChange={(event) => setValue({ ...value, parentName: event.target.value })}
            />
            <TextField
                label="Email address"
                type="email"
                value={value.emailAddress}
                onChange={(event) => setValue({ ...value, emailAddress: event.target.value })}
            />
            <TextField
                label="Mobile number"
                value={value.mobileNumber}
                onChange={(event) => setValue({ ...value, mobileNumber: event.target.value })}
            />
            <TextField
                label="Emergency contact"
                value={value.emergencyContactName}
                onChange={(event) =>
                    setValue({ ...value, emergencyContactName: event.target.value })
                }
            />
            <TextField
                ...
                onChange={(event) =>
                    setValue({ ...value, emergencyContactPhone: event.target.value })
                }
            />
        </Box>
    );
                ...
                <LeaderPageHeader
                    ...
                    actions={
                        <Stack direction={{ xs: "column", sm: "row" }} spacing={1.5}>
                            <Button variant="contained" color="secondary" onClick={openAddMember}>
                                Add Member
                            </Button>
                            <Button variant="contained" color="success" onClick={() => void load()}>
                                Refresh
                            </Button>
                        </Stack>
                    }
                                ...
                                >
                                    <Box>
                                        ...
                                        >
                                            ...
                                            />
                                            {member.section && (
                                                <Chip label={member.section} variant="outlined" size="small" />
                                            )}
                                        </Stack>
                                        <Typography sx={{ mt: 1 }}>
                                            Parent / Guardian: {member.parentName || "Not provided"}
                                        </Typography>
                                        <Typography sx={{ mt: 0.5 }}>
                                            Phone: {member.mobileNumber || "Not provided"}
                                        </Typography>
                                    </Box>
                                    <Button
                                        variant="contained"
                                            ...
                                            <Paper key={consent.consentId} variant="outlined" sx={{ p: 2 }}>
                                                ...
                                                >
                                                    <Chip
                                                        size="small"
                                                        label={
                                                            consentExpired(consent.consentTo)
                                                                ? "Expired"
                                                                : "Consent found"
                                                        }
                                                        color={
                                                ...
                                                <Typography variant="body2" color="text.secondary">
                                                    {consent.section || "No section"} · Submitted{" "}
                                                    {formatDate(consent.submittedAt)} · Consent to{" "}
                                                    {consent.consentTo || "not provided"}
                                                </Typography>
                                            </Paper>
                                        ))}
                                    </Stack>
                ...
                >
                    ...
                    <DialogContent dividers>
                        <Typography color="text.secondary" sx={{ mb: 3 }}>
                            Add a current member directly to the member register. This does not create a Join Us enquiry.
                        </Typography>
                    ...
                    <DialogActions>
                        <Button disabled={creating} onClick={() => setAddOpen(false)}>
                            Cancel
                        </Button>
                        <Button
                            variant="contained"
