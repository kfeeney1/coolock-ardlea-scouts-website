...
export default function CreateWeeklyMeetingPage(){
 ...
 const [programmeNotes,setProgrammeNotes]=useState(""); const [saving,setSaving]=useState(false); const [error,setError]=useState(""); const [canViewAll,setCanViewAll]=useState(false);
 const sections=useMemo(()=>isAdmin||canViewAll?GROUP_SECTIONS:adminProfile?.sections??[],[isAdmin,canViewAll,adminProfile?.sections]);
 useEffect(()=>{void loadWeeklyAccess().then(a=>setCanViewAll(a.canViewAll));},[]);
