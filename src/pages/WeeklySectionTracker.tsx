...
export default function WeeklySectionTracker() {
  ...
  {loading?<Box sx={{minHeight:300,display:"grid",placeItems:"center"}}><CircularProgress/></Box>:!selected?<Stack spacing={2}>
  ...
  </Stack>:<Stack spacing={2} sx={{minWidth:0}}>
    <Paper variant="outlined" sx={{p:{xs:1.5,sm:2},minWidth:0}}><Stack direction={{xs:"column",md:"row"}} spacing={1} sx={{justifyContent:"space-between",alignItems:{md:"center"}}}><Box sx={{minWidth:0}}><Typography variant="h5" sx={{fontWeight:800,overflowWrap:"anywhere"}}>{selected.section} · {displayDate(selected.meetingDate)}</Typography><Chip size="small" label={selected.status==="open"?"Open":"Closed"}/></Box><Stack direction={{xs:"column",sm:"row"}} spacing={1} useFlexGap sx={{flexWrap:"wrap"}}><Button fullWidth onClick={()=>setSelected(null)}>Meetings</Button>{!readOnly&&<Button fullWidth onClick={()=>{setCopySource(selected);setCopyDate(today);setSelected(null);}}>Copy Meeting</Button>}{!readOnly&&selected.status==="closed"&&<Button fullWidth variant="outlined" onClick={()=>void persist({...selected,status:"open"},"Meeting reopened.")}>Reopen Meeting</Button>}</Stack></Stack></Paper>
    ...
    {!readOnly&&<Paper elevation={3} sx={{position:"sticky",bottom:8,p:1.25,zIndex:2}}><Stack direction={{xs:"column",sm:"row"}} spacing={1} sx={{justifyContent:"flex-end"}}><Button fullWidth variant="outlined" onClick={()=>void save()} disabled={saving}>Save Meeting</Button>{selected.status==="open"&&<Button fullWidth variant="contained" color="success" onClick={()=>void persist({...selected,status:"closed"},"Meeting closed and added to history.")} disabled={saving}>Close Meeting</Button>}</Stack></Paper>}
  </Stack>}</Container></Box>;
}
