...
export default function WeeklySectionTracker() {
  ...
  const persist=async(next:WeeklyMeetingRecord,message:string,action="weekly-meeting-update"):Promise<boolean>=>{ if(readOnly)return false; setSaving(true);setError("");setSuccess("");try{const{id,...input}=next;await updateWeeklyMeeting(id,input);await auditWeeklyMeeting(next,action,message);setSelected(next);setSuccess(message);await refresh(access);return true;}catch(e){console.error(e);setError("Unable to save this meeting.");return false;}finally{setSaving(false);} };
  ...
  {loading?<Box sx={{minHeight:300,display:"grid",placeItems:"center"}}><CircularProgress/></Box>:!selected?<Stack spacing={2}>
    ...
    <Paper ref={editorTopRef} data-testid="weekly-meeting-editor-top" variant="outlined" sx={{p:{xs:1.5,sm:2},minWidth:0,scrollMarginTop:16}}><Stack direction={{xs:"column",md:"row"}} spacing={1} sx={{justifyContent:"space-between",alignItems:{md:"center"}}}><Box sx={{minWidth:0}}><Typography variant="h5" sx={{fontWeight:800,overflowWrap:"anywhere"}}>{selected.section} · {displayDate(selected.meetingDate)}</Typography><Chip size="small" label={selected.status==="open"?"Open":"Closed"}/></Box><Stack direction={{xs:"column",sm:"row"}} spacing={1} useFlexGap sx={{flexWrap:"wrap"}}><Button fullWidth onClick={()=>setSelected(null)}>Meetings</Button><Button fullWidth component="a" href={whatsappUrl} target="_blank" rel="noreferrer" variant="outlined" color="success" data-testid="weekly-whatsapp-share">Share in WhatsApp</Button>{!readOnly&&<Button fullWidth onClick={()=>{setCopySource(selected);setCopyDate(today);setSelected(null);}}>Copy Meeting</Button>}{!readOnly&&selected.status==="closed"&&<Button fullWidth variant="outlined" onClick={()=>void persist({...selected,status:"open"},"Meeting reopened.","weekly-meeting-reopen")}>Reopen Meeting</Button>}</Stack></Stack></Paper>
