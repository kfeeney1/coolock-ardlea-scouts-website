    ...
    <Box sx={{ minHeight: "100vh", backgroundColor: "background.default", py: { xs: 4, md: 6 } }}>
      <Container maxWidth="lg">
        ...
        <Typography color="text.secondary" sx={{ mb: 3 }}>
          Admins manage leader section assignments. A leader can belong to multiple sections. Only a Super Admin can grant or remove Admin access.
        </Typography>
        ...
        <Stack spacing={2}>
          {records.map((record) => (
            <Paper key={record.uid} variant="outlined" sx={{ p: 2.5 }}>
              ...
              {record.role === "leader" && (
                <Box sx={{ mt: 2 }}>
                  <Typography variant="subtitle2" color="secondary" sx={{ fontWeight: 800, mb: 1 }}>
                    Sections — click all that apply
                  </Typography>
                    ...
                    {sections.map((section) => {
                      ...
                      return (
                        <Chip
                          ...
                          sx={{ fontWeight: selected ? 800 : 500, px: 0.5 }}
                        />
                      );
                    })}
                  </Box>
                  <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                    Selected: {record.sections.length ? record.sections.join(", ") : "No sections"}
                  </Typography>
                </Box>
              )}
