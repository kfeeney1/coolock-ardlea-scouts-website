...
export default function LeaderAccessManagement() {
  ...
  return <Box sx={{ minHeight: "100vh", backgroundColor: "background.default", py: { xs: 4, md: 6 } }}><Container maxWidth="xl">
    ...
    <LeaderPageHeader title="Leader Access & Organisation" />
      ...
      {[selectedRecord].map((record) => <Paper key={record.uid} data-testid={`leader-access-${record.uid}`} data-section={record.organisationSection} variant="outlined" sx={[{ p: { xs: 2, md: 3 }, borderRadius: 2 }, sectionCardSx(record.organisationSection)]}>
        ...
        {record.role === "leader" && <Box sx={{ mt: 3 }} data-testid="leader-child-links">
          ...
          <Stack direction={{ xs: "column", sm: "row" }} spacing={1} sx={{ alignItems: { sm: "center" } }}>
            ...
            <Button variant="outlined" disabled={!childToLink || familyWorking} onClick={() => { setFamilyWorking(true); void setLeaderChildRelationship(record.uid, childToLink, true).then(() => { setLinkedChildIds((ids) => [...new Set([...ids, childToLink])]); setChildToLink(""); await reconcileCurrentLeaderFamilySubs(childToLink); setMessage("Leader-child relationship linked. Current Scout-year family Subs classification has been reconciled."); }).catch((e) => setError(e instanceof Error ? e.message : "Unable to link child.")).finally(() => setFamilyWorking(false)); }}>Link child</Button>
          </Stack>
          <Stack spacing={1} sx={{ mt: 1.5 }}>{linkedChildIds.map((memberId) => { const member = familyMembers.find((item) => item.id === memberId); return <Box key={memberId} sx={{ display: "flex", justifyContent: "space-between", gap: 1, alignItems: "center" }}><Typography>{member?.displayName || memberId}{member ? ` · ${member.section}` : ""}</Typography><Button color="error" size="small" disabled={familyWorking} onClick={() => { setFamilyWorking(true); void setLeaderChildRelationship(record.uid, memberId, false).then(() => { setLinkedChildIds((ids) => ids.filter((id) => id !== memberId)); await reconcileCurrentLeaderFamilySubs(memberId); setMessage("Leader-child relationship unlinked. Current Scout-year family Subs classification has been reconciled."); }).catch((e) => setError(e instanceof Error ? e.message : "Unable to unlink child.")).finally(() => setFamilyWorking(false)); }}>Unlink</Button></Box>; })}</Stack>
