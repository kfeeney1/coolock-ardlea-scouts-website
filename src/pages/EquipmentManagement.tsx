...
export default function EquipmentManagement() {
  ...
  const save = async () => {
    ...
    if (category === OTHER) {
      ...
      if (!safe) return setError("Enter the new category name.");
      if (isDuplicateEquipmentLabel(safe, categoryNames)) return setError("That category already exists. Select it from the list instead.");
    ...
    if (location === OTHER) {
      ...
      if (!safe) return setError("Enter the new storage location.");
      if (isDuplicateEquipmentLabel(safe, locationNames)) return setError("That location already exists. Select it from the list instead.");
      location = (await addEquipmentOption("locations", safe)).name;
    }
    if (!category || !location) return setError("Choose a category and storage location.");
    ...
    try {
      ...
      setError("Unable to save the equipment item.");
    } finally {
      setSaving(false);
    }
  };

  const removeLocation = async (option: EquipmentOption) => {
    if (!canDeleteEquipmentOption(option.name, items.filter((item) => !item.archived).map((item) => item.location))) return;
    try {
      await deleteEquipmentOption("locations", option);
      await refresh();
    } catch (deleteError) {
      console.error("Unable to delete location:", deleteError);
      setError("Unable to delete that storage location.");
    }
  };

  return <Box sx={{ minHeight: "100vh", backgroundColor: "background.default", py: { xs: 3, md: 5 } }}>
    <Container maxWidth="xl">
      <LeaderPageHeader title="Equipment & Stores" subtitle="Track the group's equipment, quantities, categories and storage locations." />
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(2, minmax(0, 1fr))", xl: "repeat(3, minmax(0, 1fr))" }, gap: 2 }}>
          {visibleItems.map((item) => <Paper key={item.id} variant="outlined" sx={{ p: 2.5, opacity: item.archived ? 0.65 : 1 }}>
            <Stack spacing={1.25}>
              ...
              {canManage && <Stack direction="row" spacing={1}><Button size="small" onClick={() => openEdit(item)}>Edit</Button><Button size="small" color={item.archived ? "success" : "warning"} onClick={() => void setEquipmentArchived(item, !item.archived).then(refresh)}>{item.archived ? "Restore" : "Archive"}</Button></Stack>}
            </Stack>
          </Paper>)}
        </Box>
      )}
      ...
      <Dialog open={editing !== undefined} onClose={() => !saving && setEditing(undefined)} fullWidth maxWidth="sm">
        ...
        <DialogContent dividers><Stack spacing={2} sx={{ pt: 0.5 }}>
          ...
          <TextField label="Total quantity" type="number" inputProps={{ min: 0, step: 1 }} value={form.totalQuantity} onChange={(event) => setForm({ ...form, totalQuantity: Number(event.target.value) })} />
          ...
          <TextField label="Replacement value (€)" type="number" inputProps={{ min: 0, step: "0.01" }} value={form.replacementValue ?? ""} onChange={(event) => setForm({ ...form, replacementValue: event.target.value === "" ? null : Number(event.target.value) })} />
      ...
      <Dialog open={manageLocationsOpen} onClose={() => setManageLocationsOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>Manage storage locations</DialogTitle>
        <DialogContent dividers>
          {locations.length === 0 ? <Alert severity="info">No saved locations yet. Add one by choosing Other… when adding equipment.</Alert> : <Stack spacing={1.25}>{locations.map((option) => {
            const usage = items.filter((item) => !item.archived && item.location.toLowerCase() === option.name.toLowerCase()).length;
            return <Paper key={option.id} variant="outlined" sx={{ p: 1.5, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 2 }}><Box><Typography sx={{ fontWeight: 700 }}>{option.name}</Typography><Typography variant="body2" color="text.secondary">{usage ? `${usage} equipment record${usage === 1 ? "" : "s"} currently use this location` : "Unused"}</Typography></Box><Button color="error" disabled={!canDeleteEquipmentOption(option.name, items.filter((item) => !item.archived).map((item) => item.location))} onClick={() => void removeLocation(option)}>Delete</Button></Paper>;
          })}</Stack>}
        </DialogContent>
        <DialogActions><Button onClick={() => setManageLocationsOpen(false)}>Close</Button></DialogActions>
      </Dialog>
    </Container>
  </Box>;
}
