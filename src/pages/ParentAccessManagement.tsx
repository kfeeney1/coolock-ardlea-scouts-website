                    ...
                    return <Paper key={parent.uid} data-testid={`parent-access-${parent.uid}`} variant="outlined" sx={{ p: 2.5 }}>
                        <Box sx={{ display: "flex", flexDirection: { xs: "column", md: "row" }, justifyContent: "space-between", gap: 2 }}>
                            ...
                            <Stack spacing={1} sx={{ minWidth: 190 }}>
                                ...
                                {parent.status !== "approved" && <><Button variant="contained" color="success" disabled={workingUid === parent.uid || linkedIds.length === 0} aria-describedby={linkedIds.length === 0 ? `parent-approval-help-${parent.uid}` : undefined} onClick={() => requestDecision(parent, "approved")}>{parent.matchingLeaderStatus ? "Approve & Merge Access" : parent.status === "revoked" ? "Re-enable Parent Access" : "Approve Access"}</Button>{linkedIds.length === 0 && <Typography id={`parent-approval-help-${parent.uid}`} variant="caption" color="text.secondary">Link at least one child before approving access.</Typography>}</>}
