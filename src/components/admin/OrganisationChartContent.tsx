...
export default function OrganisationChartContent() {
  ...
  if (loading) return <Box sx={{ minHeight: 260, display: "grid", placeItems: "center" }}><CircularProgress color="success" /></Box>;
  if (error) return <Alert severity="error">Unable to load the internal organisation chart.</Alert>;
  if (leaders.length === 0) return <Alert severity="info">No leaders have been added to the organisational chart yet.</Alert>;
