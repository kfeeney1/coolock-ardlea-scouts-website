...
export default function EventListPanel({ events, visibleEvents, members, loading, search, sectionFilter, statusFilter, onSearchChange, onSectionFilterChange, onStatusFilterChange, onEdit, onRoster, onEquipment, onGallery, onPrint, onExport }: Props) {
    ...
    return <>
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr 1fr", md: "repeat(5, minmax(0, 1fr))" }, gap: 2, mb: 3 }} aria-label="Event status summary">
            {[["Total", totals.total, "all"], ["Open", totals.open, "open"], ["Draft", totals.draft, "draft"], ["Closed", totals.closed, "closed"], ["Completed", totals.completed, "completed"]].map(([label, value, filter]) => {
                ...
                return <Paper
                    ...
                    onClick={() => selectStatus(filterValue)}
                    onKeyDown={(event) => {
                        if (event.key === "Enter" || event.key === " ") {
                            event.preventDefault();
                            selectStatus(filterValue);
                        }
                    }}
        ...
        <Box id="event-results" role="region" aria-label="Event results" tabIndex={-1} sx={{ scrollMarginTop: 16 }}>
            {loading ? <Box sx={{ minHeight: 300, display: "flex", justifyContent: "center", alignItems: "center" }}><CircularProgress color="success" /></Box> : <Box sx={{ display: "grid", gap: 2 }}>
                ...
                {visibleEvents.map((event) => {
                    ...
                    return <Paper key={event.id} id={`event-${event.id}`} variant="outlined" sx={{ p: 2.5, scrollMarginTop: 16 }} data-testid={`event-card-${event.id}`}>
