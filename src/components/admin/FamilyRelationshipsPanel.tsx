...
export default function FamilyRelationshipsPanel({ member, members, onChanged }: Props) {
  ...
  return <Stack spacing={2} data-testid="member-family-management">
    <Alert severity="info">
    ...
    {member.familyId && <Box><Button variant="outlined" color="error" disabled={working} onClick={() => void unlink()}>{working ? "Updating..." : "Remove from family"}</Button></Box>}
    <Box>
    ...
    <Typography variant="caption" color="text.secondary">Family relationship changes require administrator access and are written atomically with an audit record.</Typography>
  </Stack>;
}
