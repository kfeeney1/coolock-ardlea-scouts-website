...
export default function LeaderDashboardHeader() {
 ...
 return <Paper elevation={3} sx={{ p: { xs: 2.5, md: 3 }, mb: 3, borderRadius: 2, borderTop: "6px solid", borderTopColor: "secondary.main" }}>
  ...
  <Collapse in={menuOpen} timeout="auto" unmountOnExit>
   <Box id="leader-navigation" component="nav" aria-label="Leader navigation" onKeyDown={(event) => { if (event.key === "Escape") closeMenuAndRestoreFocus(); }} sx={{ mt: 1.5 }}>
    <Box data-testid="leader-navigation-mobile" sx={{ display: { xs: "block", md: "none" } }}>
     <Stack spacing={1}>
      {visibleGroups.map((group) => {
       ...
       return <Paper key={group.label} variant="outlined" sx={{ borderRadius: 2, overflow: "hidden" }}>
        <Button id={buttonId} fullWidth color="secondary" aria-expanded={expanded} aria-controls={panelId} onClick={() => setMobileGroupOpen((open) => open === group.label ? null : group.label)} endIcon={<ExpandMoreIcon sx={{ transform: expanded ? "rotate(180deg)" : "none", transition: "transform 160ms ease" }} />} sx={{ minHeight: 48, px: 1.5, justifyContent: "space-between", fontWeight: containsActive ? 800 : 700 }}>{group.label}</Button>
        <Collapse in={expanded} timeout="auto" unmountOnExit><Stack id={panelId} role="region" aria-labelledby={buttonId} spacing={0.25} sx={{ px: 1, pb: 1 }}>{group.items.map(navButton)}</Stack></Collapse>
       </Paper>;
      })}
     </Stack>
    </Box>
    <Box data-testid="leader-navigation-desktop" sx={{ display: { xs: "none", md: "grid" }, gridTemplateColumns: { md: "repeat(2, minmax(0, 1fr))", xl: "repeat(4, minmax(0, 1fr))" }, gap: 1.5, alignItems: "start" }}>
