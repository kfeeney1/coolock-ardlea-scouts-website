...
export default function LeaderDashboardHeader() {
 ...
 const dashboardActive = matchesNavPath(location.pathname, dashboardItem.path);
 return <Paper data-testid="leader-dashboard-header" elevation={3} sx={{ p: { xs: 2.5, md: 3 }, mb: 3, borderRadius: 2, borderTop: "6px solid", borderTopColor: "secondary.main", width: { xs: "calc(100vw - 32px)", md: "calc(100vw - 48px)" }, maxWidth: 1536, position: "relative", left: "50%", transform: "translateX(-50%)", boxSizing: "border-box" }}>
  ...
  <Stack direction={{ xs: "column", sm: "row" }} spacing={1}>
   {!dashboardActive && <Button component={Link} to="/leader" variant="text" color="secondary" startIcon={<HomeOutlinedIcon />} sx={{ minHeight: 48, px: 1.5, flexShrink: 0, fontWeight: 800 }}>Dashboard</Button>}
  ...
  <Collapse in={menuOpen} timeout="auto" unmountOnExit><Box id="leader-navigation" component="nav" aria-label="Leader navigation" onKeyDown={(event) => { if (event.key === "Escape") closeMenuAndRestoreFocus(); }} sx={{ mt: 1.5 }}>
