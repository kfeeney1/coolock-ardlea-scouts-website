...
export default function ProgrammeLibraryPanel({ section, activities, badgework, readOnly, onInsertActivity, onInsertBadgework }: Props) {
  ...
  return <Paper variant="outlined" sx={{ p: 2 }} data-testid="programme-library-panel">
    ...
    <Stack spacing={1.5}>
      ...
      <Typography variant="body2" color="text.secondary" data-testid="programme-library-result-count">{visibleItems.length} of {items.length} saved items</Typography>
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr auto auto" }, gap: 1 }}>
        <TextField select fullWidth label="Saved programme item" value={selectedId} onChange={(e) => setSelectedId(e.target.value)} disabled={loading || visibleItems.length === 0}>
      ...
      {items.length > 0 && visibleItems.length === 0 && !loading && <Alert severity="info">No saved programme items match the current search and filters.</Alert>}
