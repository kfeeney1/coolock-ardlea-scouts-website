  ...
  return <Stack spacing={2} data-testid="badgework-overview">
    <Paper elevation={2} sx={{ p: { xs: 2, md: 3 } }}>
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "2fr repeat(3, 1fr)" }, gap: 2 }}>
        ...
        <FormControl><InputLabel>Adventure Skill</InputLabel><Select label="Adventure Skill" value={skillFilter} onChange={(event) => setSkillFilter(event.target.value)}><MenuItem value="all">All skills</MenuItem>{adventureSkills.map((skill) => <MenuItem key={skill.id} value={skill.id}>{skill.name}</MenuItem>)}</Select></FormControl>
        <FormControl><InputLabel>Progress</InputLabel><Select label="Progress" value={progressFilter} onChange={(event) => setProgressFilter(event.target.value as BadgeworkProgressFilter)}><MenuItem value="all">All progress</MenuItem><MenuItem value="awaiting-award">Awaiting award</MenuItem><MenuItem value="in-progress">In progress</MenuItem><MenuItem value="awarded">Awarded</MenuItem><MenuItem value="not-started">Not started</MenuItem></Select></FormControl>
      </Box>
    </Paper>
