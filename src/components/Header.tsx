...
export default function Header() {
    ...
    return <AppBar data-site-sticky-header position="sticky" elevation={3} sx={{ backgroundColor: "primary.main", borderBottom: `4px solid ${brandColours.navy}` }}>
        <Toolbar sx={{ minHeight: { xs: 72, md: 82 } }}>
            ...
            <Box sx={{ display: { xs: "flex", md: "none" } }}><IconButton color="inherit" onClick={(event) => setAnchorEl(event.currentTarget)} aria-label="Open navigation menu"><MenuIcon /></IconButton><Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)}>{menuItems.map((item) => <MenuItem key={item.path} component={Link} to={item.path} replace onClick={() => setAnchorEl(null)}>{item.label}</MenuItem>)}<MenuItem component={Link} to="/join" replace onClick={() => setAnchorEl(null)} sx={{ color: "success.dark", fontWeight: 800 }}>Join Us</MenuItem><MenuItem component={Link} to="/parent" replace onClick={() => setAnchorEl(null)} sx={{ color: "primary.dark", fontWeight: 800 }}>Parent Login</MenuItem><MenuItem component={Link} to="/leader/login" replace onClick={() => setAnchorEl(null)} sx={{ color: "secondary.main", fontWeight: 800 }}>Leader Login</MenuItem></Menu></Box>
        </Toolbar>
    </AppBar>;
}
