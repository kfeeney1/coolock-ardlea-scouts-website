    ...
    return <AppBar data-site-sticky-header data-theme-surface="header" position="sticky" elevation={3} sx={{ backgroundColor: "primary.main", borderBottom: `4px solid ${brandColours.navy}` }}>
        <Toolbar sx={{ minHeight: { xs: 72, md: 82 } }}>
            ...
            <Box sx={{ display: { xs: "none", md: "flex" }, alignItems: "center", gap: 0.5 }}>
                ...
                {user ? <Button onClick={() => void handleSignOut()} disabled={signingOut} variant="contained" color="secondary" sx={{ ml: 1, boxShadow: "none" }}>{signingOut ? "Signing Out…" : "Sign Out"}</Button> : <Button component={Link} to="/leader/login" variant="contained" color="secondary" sx={{ ml: 1, boxShadow: "none" }}>Leader Login</Button>}
            </Box>
