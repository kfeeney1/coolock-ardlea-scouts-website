...
type MenuItemDefinition = { name: string; path: string; };
    ...
    return <AppBar position="sticky" elevation={3} sx={{ backgroundColor: "primary.main", borderBottom: `4px solid ${brandColours.navy}` }}>
        <Toolbar sx={{ minHeight: { xs: 72, md: 82 } }}>
            <Box component={Link} to="/" sx={{ display: "flex", alignItems: "center", textDecoration: "none", color: "inherit", flexGrow: 1, minWidth: 0 }}>
                ...
                <Box sx={{ minWidth: 0 }}><Typography variant="h6" sx={{ fontWeight: 800, lineHeight: 1.1, fontSize: { xs: "1rem", sm: "1.15rem" } }}>80th 160th</Typography><Typography variant="body2" sx={{ lineHeight: 1.2, display: { xs: "none", sm: "block" } }}>Coolock Ardlea Scout Group</Typography></Box>
            </Box>
            <Box sx={{ display: { xs: "none", md: "flex" }, alignItems: "center", gap: 0.5 }}>
                {menuItems.map((item) => <Button key={item.name} component={Link} to={item.path} color="inherit" sx={{ color: "white", px: 1.25, "&:hover": { backgroundColor: "secondary.main" } }}>{item.name}</Button>)}
