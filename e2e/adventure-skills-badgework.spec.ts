...
test.describe("Adventure Skills badgework", () => {
  ...
  test("admin can select multiple members and meeting handoff preselects attendees", async ({ page }) => {
    ...
    await page.getByRole("combobox", { name: "Level to inspect" }).click();
    await expect(page.getByRole("option", { name: "Level 6", exact: true })).toBeVisible();
    await expect(page.getByRole("option", { name: "Level 7", exact: true })).toHaveCount(0);
    await page.keyboard.press("Escape");
    await page.getByRole("combobox", { name: "Adventure Skill" }).click();
    await page.getByRole("option", { name: "Camping" }).click();
    await page.getByRole("button", { name: "Clear all filters" }).click();
    await expect(page.getByRole("button", { name: "Clear all filters" })).toHaveCount(0);
    await expect(page.getByRole("combobox", { name: "Adventure Skill" })).toHaveText(/All skills/);
    await page.getByRole("combobox", { name: "Adventure Skill" }).click();
    await page.getByRole("option", { name: "Camping" }).click();
