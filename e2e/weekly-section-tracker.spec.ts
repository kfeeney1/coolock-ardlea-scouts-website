...
test("section leader completes lifecycle with flexible planner rows and copy", async ({ page }, testInfo) => {
  ...
  await page.getByLabel("Activity 1").fill("Wide game"); await firstActivityLeader(page).check(); await page.getByLabel("Equipment 1").fill("Cones and maps"); await page.getByLabel("Start 1").fill("19:00"); await page.getByLabel("Finish 1").fill("19:25"); await page.getByLabel("Instructions / notes 1").fill("Patrol navigation challenge");
  ...
  await page.getByRole("button", { name: "Programme", exact: true }).click(); await expect(page.getByTestId("activity-plan-row")).toHaveCount(3); await expect(page.getByLabel("Activity 1")).toHaveValue("Wide game"); await expect(firstActivityLeader(page)).toBeChecked(); await expect(page.getByTestId("badgework-plan-row")).toHaveCount(2); await expect(page.getByLabel("Badgework 2")).toHaveValue("Teamwork");
  ...
  await page.getByRole("button", { name: "Programme", exact: true }).click(); await expect(page.getByLabel("Theme")).toHaveValue("Navigation Night"); await expect(page.getByTestId("activity-plan-row")).toHaveCount(3); await expect(page.getByLabel("Activity 1")).toHaveValue("Wide game"); await expect(firstActivityLeader(page)).toBeChecked(); await expect(page.getByLabel("Equipment 1")).toHaveValue("Cones and maps"); await expect(page.getByTestId("badgework-plan-row")).toHaveCount(2); await expect(page.getByLabel("Badgework 1")).toHaveValue("Adventure Skills: Pioneering");
