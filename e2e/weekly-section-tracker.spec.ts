...
test("section leader completes lifecycle with flexible planner rows, summary and retained save state", async ({ page }, testInfo) => {
  ...
  await page.getByRole("button", { name: "Injuries / Medical", exact: true }).click(); await expect(page.getByText(/Small graze during wide game/)).toHaveCount(1); await page.getByRole("button", { name: "Notes", exact: true }).click(); await expect(page.getByLabel("Additional meeting notes")).toHaveValue("");
});
...
test("group leader can create meetings across sections", async ({ page }, testInfo) => { desktopOnly(testInfo); test.skip(!password, "Configure canonical E2E password."); await login(page, "test.group.leader@example.com"); await page.goto("/leader/weekly/create"); await expect(page.getByRole("heading", { name: "Create Meeting" })).toBeVisible(); const section=page.getByLabel("Section", { exact: true }); await expect(section).toBeVisible(); await section.click(); await expect(page.getByRole("option", { name: "Beavers" })).toBeVisible(); await expect(page.getByRole("option", { name: "Rovers" })).toBeVisible(); });
