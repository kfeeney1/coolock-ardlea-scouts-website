...
test("section leader completes lifecycle with flexible planner rows and copy", async ({ page }, testInfo) => {
  ...
  await page.getByLabel("Activity 1", { exact: true }).fill("Wide game"); await firstActivityLeader(page).check(); await page.getByLabel("Equipment 1", { exact: true }).fill("Cones and maps"); await page.getByLabel("Start 1", { exact: true }).fill("19:00"); await page.getByLabel("Finish 1", { exact: true }).fill("19:25"); await page.getByLabel("Instructions / notes 1", { exact: true }).fill("Patrol navigation challenge");
  await page.getByLabel("Activity 2", { exact: true }).fill("Pioneering relay");
  await page.getByRole("button", { name: "Add activity / game", exact: true }).click(); await expect(page.getByTestId("activity-plan-row")).toHaveCount(3); await page.getByTestId("activity-plan-row").last().getByRole("button", { name: "Remove" }).click(); await expect(page.getByTestId("activity-plan-row")).toHaveCount(2); await page.getByRole("button", { name: "Add activity / game", exact: true }).click(); await page.getByLabel("Activity 3", { exact: true }).fill("Closing game"); await page.getByLabel("Programme notes").fill("Reusable opening and patrol rotation.");

  if (created) await expect(page.getByTestId("badgework-plan-row")).toHaveCount(1);
  await page.getByLabel("Badgework 1", { exact: true }).fill("Adventure Skills: Pioneering"); await page.getByLabel("Badgework instructions / notes 1", { exact: true }).fill("Stage 2 lashings");
  await page.getByRole("button", { name: "Add badgework", exact: true }).click(); await expect(page.getByTestId("badgework-plan-row")).toHaveCount(2); await page.getByTestId("badgework-plan-row").last().getByRole("button", { name: "Remove" }).click(); await expect(page.getByTestId("badgework-plan-row")).toHaveCount(1); await page.getByRole("button", { name: "Add badgework", exact: true }).click(); await page.getByLabel("Badgework 2", { exact: true }).fill("Teamwork");
