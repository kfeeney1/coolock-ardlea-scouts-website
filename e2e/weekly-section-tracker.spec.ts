...
test("section leader completes lifecycle with flexible planner rows and copy", async ({ page }, testInfo) => {
  ...
  await page.getByRole("button", { name: "Add activity / game", exact: true }).click(); await expect(page.getByTestId("activity-plan-row")).toHaveCount(3); await page.getByTestId("activity-plan-row").last().getByRole("button", { name: "Remove" }).click(); await expect(page.getByTestId("activity-plan-row")).toHaveCount(2); await page.getByRole("button", { name: "Add activity / game", exact: true }).click(); await page.getByLabel("Activity 3").fill("Closing game"); await page.getByLabel("Programme notes").fill("Reusable opening and patrol rotation.");
  ...
  await page.getByRole("button", { name: "Add badgework", exact: true }).click(); await expect(page.getByTestId("badgework-plan-row")).toHaveCount(2); await page.getByTestId("badgework-plan-row").last().getByRole("button", { name: "Remove" }).click(); await expect(page.getByTestId("badgework-plan-row")).toHaveCount(1); await page.getByRole("button", { name: "Add badgework", exact: true }).click(); await page.getByLabel("Badgework 2").fill("Teamwork");
  ...
  await page.getByRole("button", { name: "Programme", exact: true }).click(); await expect(page.getByTestId("activity-plan-row")).toHaveCount(3); await expect(page.getByLabel("Activity 1")).toHaveValue("Wide game"); await expect(page.getByLabel("Leader 1")).toContainText("Scouts Section Leader"); await expect(page.getByTestId("badgework-plan-row")).toHaveCount(2); await expect(page.getByLabel("Badgework 2")).toHaveValue("Teamwork");
  ...
  await page.getByRole("button", { name: "Programme", exact: true }).click(); await expect(page.getByLabel("Theme")).toHaveValue("Navigation Night"); await expect(page.getByTestId("activity-plan-row")).toHaveCount(3); await expect(page.getByLabel("Activity 1")).toHaveValue("Wide game"); await expect(page.getByLabel("Leader 1")).toContainText("Scouts Section Leader"); await expect(page.getByLabel("Equipment 1")).toHaveValue("Cones and maps"); await expect(page.getByTestId("badgework-plan-row")).toHaveCount(2); await expect(page.getByLabel("Badgework 1")).toHaveValue("Adventure Skills: Pioneering");
