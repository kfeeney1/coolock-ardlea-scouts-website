...
test.describe("leader journey", () => {
  ...
  test("administrator can discover the pending request and requested assignment", async ({ page }, testInfo) => {
    desktopOnly(testInfo);
    test.skip(!password, "Configure E2E_TEST_USER_PASSWORD.");
  ...
  test("approved section leader can move through members events and consent for assigned section", async ({ page }, testInfo) => {
    desktopOnly(testInfo);
    test.skip(!password, "Configure E2E_TEST_USER_PASSWORD.");
    ...
    await expect(page.getByRole("heading", { name: /Event Consent/i }).first()).toBeVisible();
    await expect(page.getByText("TEST Scout Hike")).toBeVisible();
    await expect(page.getByText("TEST Cub Weekend Camp")).toHaveCount(0);
  });
});
