...
test("SW-147 and SW-148 share one canonical source and Super Admin route", async () => {
  ...
  assert.ok(nav.includes("leaderNavGroups"));
  assert.ok(navContract.includes("System Information"));
  assert.ok(navContract.includes("superAdminOnly: true"));
