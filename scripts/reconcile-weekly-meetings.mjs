...
function activityPayload(text) {
  const items = text ? [{ id: "legacy-activity-1", activity: text, leader: "", notes: "", equipment: "", startTime: "", finishTime: "" }] : [];
  return JSON.stringify({ marker: activityMarker, items });
}
function badgeworkPayload(text) {
  const items = text ? [{ id: "legacy-badgework-1", badge: text, notes: "" }] : [];
  return JSON.stringify({ marker: badgeworkMarker, items });
}
...
for (const doc of snapshot.docs) {
  const data = doc.data();
  if (typeof data.section !== "string" || typeof data.meetingDate !== "string" || !Array.isArray(data.entries)) {
    invalid.push(`${doc.id}: missing canonical section, meetingDate or entries`);
    continue;
  }
  const alreadyCanonical = ["open", "closed"].includes(data.status)
  ...
  const plan = legacyPlan(data.notes);
  migrated += 1;
  if (!checkOnly) {
    await doc.ref.update({
      status: "closed",
      location: plan.location,
      plannedActivities: activityPayload(plan.plannedActivities),
      plannedBadgework: badgeworkPayload(plan.plannedBadgework),
      programmeNotes: programmePayload(plan.theme, plan.programmeNotes),
      notes: plan.notes,
      injuries: [],
      updatedBy: "WEEKLY_MEETING_MIGRATION",
      updatedAt: FieldValue.serverTimestamp()
    });
  }
}
if (invalid.length) {
  console.error("Weekly meeting records requiring manual repair:");
  invalid.forEach((item) => console.error(`- ${item}`));
  process.exit(1);
}
if (checkOnly && migrated) {
  console.error(`${migrated} weekly meeting record(s) still require migration.`);
  process.exit(1);
}
