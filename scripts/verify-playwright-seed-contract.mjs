...
const superAdminSeedPath = "scripts/seed-superadmin-login.mjs";
...
const superAdminSeed = readFileSync(superAdminSeedPath, "utf8");
const workflow = readFileSync(workflowPath, "utf8");
const weeklySeed = readFileSync(weeklySeedPath, "utf8");

const seededEmails = new Set([
  ...populationSeed.matchAll(/email:\s*"([^"]+@example\.com)"/g),
  ...superAdminSeed.matchAll(/email:\s*"([^"]+@example\.com)"/g)
].map((match) => match[1]));
...
for (const token of ["TEST_uid_modern_super_admin_01","modernsuperadmin@example.com","uiTheme: \"modern\""]) {
  if (!superAdminSeed.includes(token)) problems.push(`${superAdminSeedPath} must retain Modern Scout fixture token ${token}`);
}
...
console.log(`Playwright seed contract verified: ${seededEmails.size} canonical accounts including Modern Scout, ${e2eFiles.length} specs, ${membersPerSection} members per section, stable varied weekly planner history, parent-safe projections, ${requiredEquipmentIds.length} allocation-free equipment fixtures and no persistent equipment transactions.`);
