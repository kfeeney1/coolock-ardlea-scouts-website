...
async function seedPendingLeaderRequest() {
  ...
  await db.collection("leaderRegistrationRequests").doc(pendingLeader.uid).set({ uid: pendingLeader.uid, fullName: pendingLeader.displayName, email: pendingLeader.email, mobileNumber: "0872000090", requestedRole: "Section Leader", requestedSection: "Cubs", reason: "TEST DATA ONLY. Pending leader journey request.", privacyConfirmed: true, status: "pending", submittedAt: FieldValue.serverTimestamp(), reviewedAt: null, reviewedBy: "", reviewNote: "", testData: true, testSeed: "stage8-role-demo", createdBySeed: "TEST_SEED" }, { merge: true });
}
