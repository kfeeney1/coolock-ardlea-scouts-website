...
const superAdmins = [
  ...
  },
  {
    ...
    email: "modernsuperadmin@example.com",
    displayName: "Modern Scout Super Admin",
    uiTheme: "modern"
  }
];

async function upsertSuperAdmin({ uid, email, displayName, uiTheme }) {
  ...
  await db.collection("organisationLeadership").doc(uid).set({
    ...
    organisationOrder: uiTheme === "modern" ? 92 : 91,
  ...
  console.log(`Super-admin login seeded: ${email} (${uiTheme} theme)`);
}

for (const account of superAdmins) await upsertSuperAdmin(account);
