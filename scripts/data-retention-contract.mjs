...
export const DATA_RETENTION_CONTRACT = Object.freeze([
  ...
  policy("consentReminderDeliveries", "consent-communications", DATA_SENSITIVITY.PERSONAL, RETENTION_DISPOSITIONS.MANUAL_REVIEW, "consent-reminder-purpose-ended", "Reminder delivery evidence contains member and recipient identifiers plus delivery lifecycle metadata, but no medical contents. Retention should be reviewed with the consent communications purpose before deletion."),
  ...
  policy("leaderChildRelationships", "identity-finance", DATA_SENSITIVITY.PERSONAL, RETENTION_DISPOSITIONS.MANUAL_REVIEW, "leader-or-member-offboarding", "Leader-child links are canonical personal relationships used to determine current Subs classification. They require explicit offboarding and finance-governance review; no retention period or automatic deletion is introduced."),
  ...
  policy("operationalMonitoring", "operations", DATA_SENSITIVITY.INTERNAL, RETENTION_DISPOSITIONS.CONFIGURATION, "monitoring-evidence-superseded", "Privacy-safe operational observations are bounded diagnostic/configuration evidence. Superseded observations may be retired deliberately; no automatic destructive retention is introduced."),
  ...
  policy("policyDocuments", "policy-publication", DATA_SENSITIVITY.INTERNAL, RETENTION_DISPOSITIONS.MANUAL_REVIEW, "policy-withdrawn-or-superseded", "Policy catalogue metadata and version pointers require explicit SW-80 lifecycle review; Block E does not invent a retention period or automatically delete superseded policy artefacts."),
  ...
  policy("subsAccounts", "finance", DATA_SENSITIVITY.SENSITIVE, RETENTION_DISPOSITIONS.NO_ROUTINE_DELETE, "finance-governance-review", "Scout-year family subs accounts snapshot shared financial obligations and classification provenance and must remain available for audit rather than being routinely deleted."),
